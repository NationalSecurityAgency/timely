...
import timely.common.configuration.RestClientProperties;
...
@Order(100) // execute this after standard customizers so we're sure to overwrite the client
            // request factory
@ConditionalOnBean(name = "outboundJDKSslContext")
public class ClientCertRestTemplateCustomizer implements RestTemplateCustomizer {
    ...
    protected HttpClientBuilder customizeHttpClient(HttpClientBuilder httpClientBuilder, SSLContext sslContext) {
        ...
        // TODO: We're allowing all hosts, since the cert presented by the service we're
        // calling likely won't match its hostname (e.g., a docker host name)
        // Instead, we could list the expected cert as a property (or use our server
        // cert), and verify that the presented name matches.
