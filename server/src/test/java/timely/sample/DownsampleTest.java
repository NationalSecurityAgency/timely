...
public class DownsampleTest {
    ...
    public void simple() {
        ...
        i = 0;
        for (Sample sample : dsample) {
            assertEquals((1 + 2 + 3 + 4) * Math.min(7, (100 - (10 + i * 7))), sample.value, .0001);
        ...
        for (Sample sample : dsample) {
            if (i == 0) {
                assertEquals(2.25, sample.value, 0.01);
            } else {
                assertEquals(4.5, sample.value, 0.001);
            }
    ...
    public void testCounterToRate() throws Exception {
        ...
        Downsample result = Downsample.combine(Collections.singleton(ds1), counter);
        int count = 0;
        for (Sample s : result) {
