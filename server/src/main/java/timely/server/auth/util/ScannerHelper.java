package timely.server.auth.util;
...
import timely.server.auth.ConfigurableVisibilityFilter;
import timely.server.auth.ScannerBaseDelegate;

public class ScannerHelper {

    private static final Logger log = LoggerFactory.getLogger(ScannerHelper.class);
    ...
    protected static void addVisibilityFilters(Iterator<Authorizations> iter, ScannerBase scanner) {
        for (int priority = 10; iter.hasNext(); priority++) {
            ...
            if (scanner instanceof ScannerBaseDelegate) {
                ...
                log.warn("Adding system visibility filter to non-wrapped scanner {}.", scanner.getClass(), new Exception());
