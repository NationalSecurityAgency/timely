...
def analytics(timelyMetric, notebook=False, alertConfig={}):
    ...
    if includeColRegex is not None:
        graphDF = graphDF.select(lambda x: not (re.search(includeColRegex, x)), axis=1)
    ...
    for i in graphDF_avg.columns:
        ...
        graphDF_avg = graphDF_avg.rename(columns={col:col+'_avg'})

    graphDF = Timely.unpivot(graphDF, timelyMetric.metric, groupByColumn=groupByColumn)
    #Timely.graph(graphDF, timelyMetric.metric, sample=sample, groupByColumn=groupByColumn, notebook=notebook)

    graphDF_avg = Timely.unpivot(graphDF_avg, timelyMetric.metric, groupByColumn=groupByColumn)
    #Timely.graph(graphDF_avg, timelyMetric.metric, sample=sample, groupByColumn=groupByColumn, notebook=notebook)
    ...
    if not combined.empty:
        ...
        Timely.graph(combined, timelyMetric.metric, sample=sample, groupByColumn=groupByColumn, notebook=notebook)