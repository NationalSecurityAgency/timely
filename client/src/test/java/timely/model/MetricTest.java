...
public class MetricTest {
    ...
    public void testJson() throws IOException {
        ...
        String expectedJson = "{\"name\":\"m1\",\"timestamp\":" + ts + ",\"measure\":1.0,\"tags\":[{\"k1\":\"v1\"}]}";

        Metric m1 = Metric.newBuilder().name("m1").tag("k1", "v1").value(now, 1.0).build();

        Metric expectedMetric = mapper.readValue(expectedJson, Metric.class);

        assertTrue(m1.equals(expectedMetric));

        expectedJson = "{\"name\":\"m1\",\"tags\":[{\"k1\":\"v1\"}],\"timestamp\":" + ts + ",\"measure\":5.0}";
        expectedMetric = mapper.readValue(expectedJson, Metric.class);

        assertEquals(expectedMetric.getValue().getTimestamp(), now);
